...
当前代码已经实现完整 remesh 主流程。当前工作重点是：用更多真实样本验证 `region_table.csv` 的稳定性，并修复不稳定 region。
...
旧的 `T001_L` 已从当前主线移除。不要再把旧 T001 输出作为当前 12 区域方案的有效结果使用。

当前 `config/region_table.csv` 包含 12 个 region，字段为：
...
```text
T076_L / T008:
unmapped_count = 24 / 45
patch_face_count = 252
status = FAIL
```

这说明失败主要来自 patch/UV 覆盖不足，而不是采样点数量不够。
...
3. 三条 boundary path 可能形成过窄 patch，导致 UV 覆盖不足。
4. patch component 选择策略可能需要更强的面积/覆盖率约束。
...
```text
至少找到若干 region，使它们在 4 个真实样本上全部 PASS。
```
...
优先统计每个 region 在 4 个样本中的 PASS 数。共同 PASS 的 region 才能进入 W3 候选。
