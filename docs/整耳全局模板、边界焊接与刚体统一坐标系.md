...
```powershell
python scripts/align_whole_ear.py --whole_ear_dir output/whole_ear_r24/weld_repaired --landmarks_dir data/landmarks --out_dir output/whole_ear_r24/aligned_reference_weld_repaired --alignment_mode fixed_reference --reference_sample MQ_S076L
```
...
## 10. 历史验证批次的真实结果（旧 T 命名数据集）

> 历史验证记录：本节保留旧样本标签和结果，不能作为当前 MQ 批次的准入结论或固定参考耳设置。
