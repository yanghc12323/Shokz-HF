...
该命令自动发现 mesh/landmark 配对样本，并在单样本失败时继续处理其他样本。批次总表、统计汇总和日志写入 `output/pipeline_runs/<时间戳>/`；如需缩短运行时间，可使用 `--skip-remesh-qc` 跳过耗时的区域 QC 图，但不会跳过 remesh、Weld、对齐或 PCA。
